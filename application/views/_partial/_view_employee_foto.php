...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $('#form_edit_foto').submit(function(e){
      ...
      var urladd = "<?php echo base_url('employee/up_foto');?>";
      submitAjaxFile(urladd,data_add,null,null,null);

      var nik = '<?php echo $this->codegenerator->decryptChar($this->uri->segment(3)); ?>';
      var data={nik:nik};
  ...
  function do_reset_foto(){
    if($("#form_reset_foto")[0].checkValidity()) {
      submitAjax("<?php echo base_url('employee/res_foto')?>",'reset','form_reset_foto',null,null);
