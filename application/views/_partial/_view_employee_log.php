...
<script type="text/javascript">
  ...
  function log(){
    ...
    $('#table_data_log').DataTable( {
      ajax: {
        url: "<?php echo base_url().'employee/emp_log/view_all/'.$profile['nik']; ?>",
        type: 'POST'
      },
      scrollX: true,
      columnDefs: [
  ...
  function do_delete_all(){
    if($("#form_delete")[0].checkValidity()) {
      submitAjax("<?php echo base_url('employee/del_log')?>",'delete_log','form_delete',null,null);
      $('#table_data_log').DataTable().ajax.reload(function (){
        Pace.restart();
      });
	    var callback=getAjaxData("<?php echo base_url().'employee/emp_log/view_one/'.$profile['nik']; ?>",null);
