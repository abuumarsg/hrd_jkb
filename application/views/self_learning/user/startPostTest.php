  				...
  				<div class="col-md-6 col-lg-7 order-md-1">
  					<div class="row">
  						<div class="col-12">
  							<div class="post-description-full">
  								<div class="mb-3">
  									<h1 class="h3">Anda telah selesai mengerjakan soal Post Test, anda dapat mereview atau merevisi jawaban anda selama waktu masih tersedia</h1>
									<br>
									<small><i>Jika anda tidak merevisi jawaban anda, berarti anda akan mengakhiri sesi post test dan anda dapat langsung ke langkah selanjutnya dengan klik tombol dibawah.</i></small>
									<br>
...
<script type="text/javascript">
	...
	function allViews()
	{
		...
		var cbx = getAjaxData("<?php echo base_url('learning/startPostTest')?>", { id:id,kode_materi:kode });
		...
		if(status == 'selesai'){
			...
			doEndRingkasan();
		}else if(status == 'posttest'){
			belumSelesai();
		}else if(status == 'selesai_posttest'){
			selesai();
		}else if(status == 'hasil_posttest'){
			doEndLearning();
		}else if(status == 'learn_project'){
			doLearnProject();
	...
	function doSelectAnswer(answer)
	{
    	...
    	submitAjaxCall("<?php echo base_url('learning/inputAnswerPost')?>", datax, 'status');
		allViews();
	}
	function goAnswer(answer)
	{
		...
		if(answer > jumlah_jawaban){
			...
			var id = $('#id_materi').val();
			var kode = $('#kode_materi').val();
			var cbx = getAjaxData("<?php echo base_url('learning/goAnswerBeforePost')?>", { id:id,kode_materi:kode,answer:answer });
			$('#kode_soal').val(cbx['kode_soal']);
			$('#percobaan').val(cbx['percobaan']);
			$('#nomorx').val(cbx['nomor']);
			$('#timeToShow').html(cbx['timeToShow']);
	...
	function doEndLearning() {
		...
		$.redirect("<?php echo base_url('learning/pagesViewRingkasanPost'); ?>", datac,);
	}
	function doLearnProject() {
		var id_materi = $('#id_materi').val();
		var percobaan = $('#percobaan').val();
		var kode_materi = $('#kode_materi').val();
		var datac = { id_materi: id_materi, percobaan: percobaan, kode_materi: kode_materi, };
		$.redirect("<?php echo base_url('learning/viewLearnProject'); ?>", datac,);
	}
