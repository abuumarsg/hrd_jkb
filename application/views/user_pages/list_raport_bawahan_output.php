  						...
  						<div class="row"> 
  							<div class="col-md-12">
  								<div class="callout callout-info"><label><i class="fa fa-info-circle"></i> Bantuan</label><br>Pilih
  									NIK untuk melihat Raport Agenda Penilaian KPI</div>
  ...
  <script type="text/javascript">
  	$(document).ready(function () {
		var max_month="<?=((isset($periode))?count($periode):0)?>"; 
  		var t= $('#table_data').DataTable({
  			ajax: {
  				url: "<?php echo base_url('kagenda/list_raport_bawahan_output/view_all/')?>",
          type: 'POST',
          data:{kode:"<?= $this->uri->segment(3);?>",tabel:"<?=$this->codegenerator->encryptChar($tabel)?>"}
  			...
  			columnDefs: [
				...
				},
				{
					...
					width: '5%',
					render: function (data, type, full, meta) {
						var kodex=9+parseInt(max_month);
						return '<a href="<?php echo base_url("kpages/view_raport_bawahan_output/");?>' + full[kodex] +'/'+full[0]+'" target="_blank">' + data + '</a>';
					}
				},
				{
					targets: [2,3],
  	...
  	function do_rekap() {
		  window.open("<?= base_url('kagenda/rekap_kpi_bawahan/'.$this->uri->segment(3))?>/?bulan=<?=$this->codegenerator->encryptChar($periode)?>");
  	}
