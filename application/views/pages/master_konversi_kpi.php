  <div class="content-wrapper">
  	<section class="content-header">
  		<h1>
  			...
  			<small>Konversi Nilai KPI</small>
  		</h1>
  		<ol class="breadcrumb">
  			<li><a href="<?php echo base_url('pages/dashboard');?>"><i class="fas fa-tachometer-alt"></i> Dashboard</a></li>
  			<li class="active">Master Konversi Nilai KPI</li>
  		</ol>
  	</section>
  			...
  			<div class="col-md-12">
  				<div class="box box-info">
  					<div class="box-header with-border">
  						<h3 class="box-title"><i class="fa fa-filter"></i> Daftar Konversi KPI</h3>
  									...
  									<div class="collapse" id="add">
  										...
  										<div class="col-md-8">
  											<form id="form_add">
												  ...
												  <div class="form-group clearfix">
  													<label>Huruf</label>
  													<input type="text" placeholder="Masukkan Huruf" name="huruf" id="data_huruf_add" class="form-control" required="required">
  												</div>
  												<div class="form-group clearfix">
  													<label>Warna</label>
  ...
  //wajib diisi
  var table="master_konversi_kpi";
  var column="id_konversi_kpi";
  $(document).ready(function(){
  	refreshCode();
  	$('#table_data').DataTable( {
  		ajax: {
  			url: "<?php echo base_url('master/master_konversi_kpi/view_all/')?>",
  			type: 'POST',
  ...
  function refreshCode() {
  	kode_generator("<?php echo base_url('master/master_konversi_kpi/kode');?>",'data_kode_add');
  }
  function view_modal(id) {
  	var data={id_konversi_kpi:id};
  	var callback=getAjaxData("<?php echo base_url('master/master_konversi_kpi/view_one')?>",data);  
  	$('#view').modal('show');
  	$('.header_data').html(callback['nama']);
  	$('#data_kode_view').html(callback['kode_konversi_kpi']);
  	$('#data_name_view').html(callback['nama']);
  	$('#data_jenis_view').html(callback['jenis_kpi']);
  ...
  function edit_modal() {
  	...
  	$('#data_kode_edit_old').val(callback['kode_konversi_kpi']);
  	$('#data_kode_edit').val(callback['kode_konversi_kpi']);
  	$('#data_name_edit').val(callback['nama']);
  	$('#data_jenis_edit').val(callback['jenis_kpi']).trigger('change');
  ...
  function do_edit(){
  	if($("#form_edit")[0].checkValidity()) {
  		submitAjax("<?php echo base_url('master/edt_konversi_kpi')?>",'edit','form_edit',null,null);
  		$('#table_data').DataTable().ajax.reload(function (){Pace.restart();},false);
  	}else{
  		notValidParamx();
  	} 
  ...
  function do_add(){
  	if($("#form_add")[0].checkValidity()) {
  		submitAjax("<?php echo base_url('master/add_konversi_kpi')?>",null,'form_add',null,null);
  		$('#table_data').DataTable().ajax.reload(function (){Pace.restart();},false);
  		$('#form_add')[0].reset();
  		refreshCode();
  	}else{
