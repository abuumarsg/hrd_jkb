 <div class="content-wrapper">
 	<section class="content-header">
 		<h1>
 			...
 			<small>Periode Penggajian Harian</small>
 		</h1>
 		<ol class="breadcrumb">
 			<li><a href="<?php echo base_url('pages/dashboard');?>"><i class="fas fa-tachometer-alt"></i> Dashboard</a></li>
 			<li class="active">Periode Penggajian Harian</li>
 		</ol>
 	</section>
 			...
 			<div class="col-md-12">
 				<div class="box box-success">
 					<div class="box-header with-border">
 						<h3 class="box-title"><i class="fa fa-list"></i> Daftar Periode Penggajian Harian</h3>
											...
											<div class="col-md-8">
												<form id="form_add">
													<div class="form-group">
														...
														<input type="text" placeholder="Masukkan Kode Periode Penggajian Harian" id="data_kode_add" name="kode" class="form-control" required="required" value="" readonly="readonly">
													</div>
													<div class="form-group">
														<label>Nama Periode Penggajian</label>
														<input type="text" placeholder="Masukkan Nama Periode Penggajian Harian" id="data_name_add" name="nama" class="form-control field" required="required">
													</div>
													<div class="form-group">
														<label>Tanggal</label>
													...
													<div class="form-group">
														<label>Sistem Penggajian Harian</label>
										...
										<div class="collapse" id="copy">
											...
											<div class="col-md-8">
												<form id="form_copy">
													...
													<div class="form-group">
														<label>Nama Periode Penggajian</label>
														<input type="text" placeholder="Masukkan Nama Penggajian Harian" id="data_name_copy" name="nama" class="form-control field" required="required">
													</div>
													<div class="form-group">
														<label>Tanggal</label>
 						...
 						<div class="row">
 							<div class="col-md-12">
 								<table id="table_data" class="table table-bordered table-striped table-responsive" width="100%">
								    <div class="callout callout-info"><label><i class="fa fa-info-circle"></i> Bantuan</label><br>Dengan <b>MENGHAPUS</b> Periode Penggajian Harian otomatis juga akan menghapus Data Penggajian Harian yang belum di selesaikan</div>
 									<thead>
 										<tr>
 											<th>No.</th>
 											<th>Kode Periode</th>
...
<div id="delete" class="modal fade" role="dialog">
 ...
 <script type="text/javascript">
  ...
  var table="data_periode_penggajian_harian";
  var column="id_periode_penggajian_harian";
  $(document).ready(function(){
  	$('#table_view').DataTable();
  	$('#table_data').DataTable( {
  		ajax: {
  			url: "<?php echo base_url('master/periode_penggajian_harian/view_all/')?>",
  			type: 'POST',
	...
	select_data('data_sistem_penggajian_edit',url_select,'master_sistem_penggajian','kode_master_penggajian','nama');
  	$('#btn_tambah').click(function(){
  		...
  		if(key == 0){
			unsetoption('data_sistem_penggajian_add',['BULANAN']);
  			$('#data_name_add').val('');
  			$('#data_sistem_penggajian_add').val('HARIAN').trigger('change');
  			refreshCode();
  ...
  function refreshCode() {
  	kode_generator("<?php echo base_url('master/periode_penggajian_harian/kode');?>",'data_kode_add');
	// $('#data_periode_copy').val('').trigger('change');
	// $('#data_name_copy').val('');
	// $('#data_mulai_copy').val('');
  }
  function periode_lama(kode, send) {
  	var data={kode_periode_penggajian_harian:kode};
  	var callback=getAjaxData("<?php echo base_url('master/periode_penggajian_harian/periode_lama')?>",data);  
  	if(kode == ''){
	...
	function view_modal(id) {
		...
		var data = {
			id_periode_penggajian_harian: id,
			mode: 'view'
		};
		var callback = getAjaxData("<?php echo base_url('master/periode_penggajian_harian/view_one')?>", data);
		$('#view').modal('show');
  	...
  	function edit_modal() {
  		...
  		var data={id_periode_penggajian_harian:id, mode: 'edit'};
  		var callback=getAjaxData("<?php echo base_url('master/periode_penggajian_harian/view_one')?>",data); 
  		$('#view').modal('toggle');
  		setTimeout(function () {
  			$('#edit').modal('show');
  	...
  	function delete_modal(id) {
		var data={id_periode_penggajian_harian:id, mode: 'view'};
		var callback=getAjaxData("<?php echo base_url('master/periode_penggajian_harian/view_one')?>",data);
	...
	function do_delete(){
		if($("#form_delete")[0].checkValidity()) {
			submitAjax("<?php echo base_url('master/del_periode_penggajian_harian')?>",'delete','form_delete',null,null);
	...
	function do_edit() {
		if ($("#form_edit")[0].checkValidity()) {
			submitAjax("<?php echo base_url('master/edt_periode_penggajian_harian')?>", 'edit', 'form_edit', null, null);
			$('#table_data').DataTable().ajax.reload();
		} else {
			notValidParamx();
		}
	...
	function do_add() {
		if ($("#form_add")[0].checkValidity()) {
			submitAjax("<?php echo base_url('master/add_periode_penggajian_harian')?>", null, 'form_add', "<?php echo base_url('master/periode_penggajian_harian/kode');?>", 'data_kode_add');
			$('#table_data').DataTable().ajax.reload(function () {
				Pace.restart();
			});
			$('#form_add')[0].reset();
 	...
 	function do_copy() {
 		if ($("#form_copy")[0].checkValidity()) {
 			submitAjax("<?php echo base_url('master/copy_periode_penggajian_harian')?>", null, 'form_copy',null,null);
 			$('#table_data').DataTable().ajax.reload(function () {
 				Pace.restart();
 			});
 			refreshCode();
