 <div class="content-wrapper">
 	<section class="content-header">
 		<h1>
 			<i class="fa far fa-credit-card"></i> Penggajian <small>Periode Lembur</small>
 		</h1>
 		<ol class="breadcrumb">
 			<li><a href="<?php echo base_url('pages/dashboard');?>"><i class="fas fa-tachometer-alt"></i> Dashboard</a></li>
 			<li class="active">Periode Lembur</li>
 		</ol>
 	</section>
 			...
 			<div class="col-md-12">
 				<div class="box box-success">
 					<div class="box-header with-border">
 						<h3 class="box-title"><i class="fa fa-list"></i> Daftar Periode Lembur</h3>
											...
											<div class="col-md-12">
												<div class="pull-left">
													<?php if (in_array($access['l_ac']['add'], $access['access'])) {
														echo '<button class="btn btn-success" href="#add" id="btn_tambah" type="button" data-toggle="collapse" data-parent="#accordion" aria-expanded="false" aria-controls="import"><i class="fa fa-plus"></i> Tambah Periode</button>
											...
											<div class="col-md-8">
												<form id="form_add">
													<div class="form-group">
														...
														<input type="text" placeholder="Masukkan Kode Periode Lembur" id="data_kode_add" name="kode" class="form-control" required="required" value="" readonly="readonly">
													</div>
													<div class="form-group">
														<label>Nama Periode Lembur</label>
														<input type="text" placeholder="Masukkan Nama Periode Lembur" id="data_name_add" name="nama" class="form-control field" required="required">
													</div>
													<div class="form-group">
														<label>Tanggal</label>
										...
										<div class="collapse" id="copy">
											...
											<div class="col-md-8">
												<form id="form_copy">
													...
													<div class="form-group">
														<label>Nama Periode Lembur</label>
														<input type="text" placeholder="Masukkan Nama Periode Lembur" id="data_name_copy" name="nama" class="form-control field" required="required">
													</div>
													<div class="form-group">
														<label>Tanggal</label>
 						...
 						<div class="row">
 							<div class="col-md-12">
 								...
 								<table id="table_data" class="table table-bordered table-striped table-responsive" width="100%">
 									<div class="callout callout-info"><label><i class="fa fa-info-circle"></i> Bantuan</label><br>Dengan
 										<b>MENGHAPUS</b> Periode Penggajian Lembur otomatis juga akan menghapus Data Penggajian Lembur yang
 										belum di selesaikan</div>
 									<thead>
 										<tr>
 											<th>No.</th>
 											<th>Kode Periode</th>
 		...
 		<div class="modal-content">
 			...
 			<form id="form_delete">
 				<div class="modal-body text-center">
 					...
 					<input type="hidden" id="data_kode_periode_delete" name="kode_periode">
 					<p>Data mungkin terkain dengan data lain.</p>
 ...
 <script type="text/javascript">
 	...
 	var table = "data_periode_lembur";
 	var column = "id_periode_lembur";
 	$(document).ready(function () {
 		$('#table_data_detail').DataTable();
 		refreshCode();
 		$('#table_data').DataTable({
 			ajax: {
 				url: "<?php echo base_url('master/periode_lembur/view_all/')?>",
 				type: 'POST',
 				data: {
 		...
 		select_data('data_sistem_penggajian_add', url_select, 'master_sistem_penggajian', 'kode_master_penggajian',
 			'nama');
 		select_data('data_sistem_penggajian_edit', url_select, 'master_sistem_penggajian', 'kode_master_penggajian',
 			'nama');
 	...
 	function refreshCode() {
 		kode_generator("<?php echo base_url('master/periode_lembur/kode');?>", 'data_kode_add');
 	...
 	function periode_lama(kode, send) {
 		...
 		var callback = getAjaxData("<?php echo base_url('master/periode_lembur/periode_lama')?>", data);
 		if (kode == '') {
 	...
 	function view_modal(id) {
 		...
 		var data = {
 			id_periode_lembur: id,
 			mode: 'view'
 		};
 		var callback = getAjaxData("<?php echo base_url('master/periode_lembur/view_one')?>", data);
 		$('#view').modal('show');
 		$('.header_data').html(callback['nama']);
 		$('#data_kode_view').html(callback['kode_periode_lembur']);
 	...
 	function edit_modal() {
 		...
 		$('#data_kode_edit_old').val(callback['kode_periode_lembur']);
 		$('#data_kode_edit').val(callback['kode_periode_lembur']);
 	...
 	function delete_modal(id) {
 		...
 		$('#data_kode_periode_delete').val(callback['kode_periode_lembur']);
 	}
 	...
 	function do_edit() {
 		if ($("#form_edit")[0].checkValidity()) {
 			submitAjax("<?php echo base_url('master/edt_periode_lembur')?>", 'edit', 'form_edit', null, null);
 			$('#table_data').DataTable().ajax.reload();
 		} else {
 			notValidParamx();
 		}
 	...
 	function do_add() {
 		if ($("#form_add")[0].checkValidity()) {
 			submitAjax("<?php echo base_url('master/add_periode_lembur')?>", null, 'form_add',
 				"<?php echo base_url('master/periode_penggajian/kode');?>", 'data_kode_add');
 	...
 	function do_copy() {
 		if ($("#form_copy")[0].checkValidity()) {
 			submitAjax("<?php echo base_url('master/copy_periode_lembur')?>", null, 'form_copy',null,null);
 			$('#table_data').DataTable().ajax.reload(function () {
 				Pace.restart();
 			});
 			refreshCode();
 	...
 	function do_delete() {
 		submitAjax("<?php echo base_url('master/delete_periode_lembur')?>", 'delete', 'form_delete', null, null);
 		$('#table_data').DataTable().ajax.reload(function () {
 			Pace.restart();
 		});
 	}
