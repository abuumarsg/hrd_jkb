<div class="content-wrapper">
    <section class="content-header">
      <h1>
        <a href="<?php echo base_url('pages/input_kpi_value/'.$this->uri->segment(3)); ?>" title="Kembali"><i class="fas fa-chevron-circle-left"></i></a>
        <i class="fa fa-edit"></i> Input KPI
        <small><?php echo $nama; ?></small>
      </h1>
      <ol class="breadcrumb">
        <li><a href="<?php echo base_url('pages/dashboard');?>"><i class="fas fa-tachometer-alt"></i> Dashboard</a></li>
        <li><a href="<?php echo base_url('pages/data_input_kpi');?>"><i class="fa fa-calendar"></i> Daftar Agenda</a></li>
        <li><a href="<?php echo base_url('pages/view_employee_kpi/'.$this->codegenerator->encryptChar($kode));?>"><i class="fa fa-users"></i> Daftar Karyawan</a></li>
...
<script src="<?php echo base_url('asset/bower_components/jquery/dist/jquery.min.js');?>"></script>
  ...
  function dataTable() {
      $('#table_data').DataTable( {
        ajax: {
          url: "<?php echo base_url('agenda/view_kpi_input/view_all/'.$this->codegenerator->encryptChar($kode).'/'.$this->uri->segment(4))?>",
          type: 'POST',
  ...
  function checkpoin(jenis,$kx) {
   var oTable = $("#table_data_"+jenis).dataTable();
   var matches = [];
   var checkedcollection = oTable.$(".poin_"+$kx+"_"+jenis, { "page": "all" });
   ...
   $("input[name='poin"+$kx+"_hidden_"+jenis+"']").val(matches);
  }

  function checkIdTasks(jenis) {
   var oTable = $("#table_data_"+jenis).dataTable();
   var matches = [];
   var checkedcollection = oTable.$("input[name='id_task_"+jenis+"[]']", { "page": "all" });
   checkedcollection.each(function (index, elem) {
      matches.push($(elem).val());
   });
   $("input[name='id_tasks_hidden_"+jenis+"']").val(matches);
  }
   function do_add(){
    checkIdTasks('wajib');
    checkIdTasks('rutin');
    checkIdTasks('tambahan');
    for (var i = 1; i <=4; i++) {
      checkpoin('wajib',i);
      checkpoin('rutin',i);
      checkpoin('tambahan',i);
    }
    setTimeout(function () {
      submitAjax("<?php echo base_url('agenda/add_input_kpi')?>",null,'form_add',null,null);
    },600); 
   }
