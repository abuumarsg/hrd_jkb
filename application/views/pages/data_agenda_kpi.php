  <div class="content-wrapper">
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Agenda KPI Output</li>
      </ol>
    </section>
        ...
        <div class="col-md-12">
          <div class="box box-danger">
            <div class="box-header with-border">
              <h3 class="box-title"><i class="fa fa-list"></i> Daftar Agenda KPI Output</h3>
                      ...
                      <div class="col-md-8">
                        <form id="form_add">
                          <div class="form-group">
                            <label>Kode Agenda KPI</label>
                            <input type="text" placeholder="Masukkan Kode Agenda KPI" name="kode" id="data_kode_add" class="form-control" value="" required="required" readonly="readonly">
                          </div>
                          <div class="form-group"> 
                            <label>Nama Agenda KPI</label>
                            <input type="text" placeholder="Masukkan Nama Agenda KPI" name="nama" id="data_name_add" class="form-control" required="required">
                          </div>
                          <div class="form-group"> 
...
<!-- view -->
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="form_edit">
            ...
            <div class="form-group">
              <label>Kode Agenda KPI</label>
              <input type="text" placeholder="Masukkan Kode Agenda KPI" id="data_kode_edit" name="kode" value="" class="form-control" required="required" readonly="readonly">
            </div>
            <div class="form-group">
              <label>Nama Agenda</label>
              <input type="text" placeholder="Masukkan Nama Agenda KPI" id="data_name_edit" name="nama" value="" class="form-control" required="required">
            </div>
            <div class="form-group">
              <label>Tanggal</label>
              <div class="has-feedback">
  ...
  $(document).ready(function(){ 
    ...
    $('#table_data').DataTable( {
      ajax: {
        url: "<?php echo base_url('agenda/agenda_kpi/view_all')?>",
        type: 'POST',
    ...
    $('#btn_add_collapse').click(function(){
      select_data('data_konsep_add',url_select,'concept_kpi','kode_c_kpi','nama');
  ...
  function refreshCode() {
    kode_generator("<?php echo base_url('agenda/agenda_kpi/kode');?>",'data_kode_add');
  }
  function view_modal(id) {
    var data={id_a_kpi:id};
    var callback=getAjaxData("<?php echo base_url('agenda/agenda_kpi/view_one')?>",data);  
    $('#view').modal('show');
    $('.header_data').html(callback['nama']);
    $('#data_kode_view').html(callback['kode_a_kpi']);
  ...
  function validate_modal(id,st) {
    ...
    var callback=getAjaxData("<?php echo base_url('agenda/agenda_kpi/view_one')?>",data);
    $('#validate').modal('show');
    $('#data_kode').val(callback['kode_a_kpi']);
    $('#data_status').val(st);
    if (st == 0) {
  ...
  function do_status(id,data) {
    ...
    var where={id_a_kpi:id};
    var datax={table:table,where:where,data:data_table};
    submitAjax("<?php echo base_url('agenda/stt_agenda_kpi')?>",null,datax,null,null,'status');
  ...
  function do_validate(){
    if($("#form_validate")[0].checkValidity()) {
      submitAjax("<?php echo base_url('agenda/val_agenda_kpi')?>",'validate','form_validate',null,null);
  ...
  function do_edit(){
    if($("#form_edit")[0].checkValidity()) {
      submitAjax("<?php echo base_url('agenda/edt_agenda_kpi')?>",'edit','form_edit',null,null);
      $('#table_data').DataTable().ajax.reload(function (){Pace.restart();},false);
    }else{
      notValidParamx();
    } 
  ...
  function do_add(){
    if($("#form_add")[0].checkValidity()) {
      show_loader();
      submitAjax("<?php echo base_url('agenda/add_agenda_kpi')?>",null,'form_add',null,null,'wload');
      $('#table_data').DataTable().ajax.reload(function (){Pace.restart();},false);
      $('#form_add')[0].reset();
