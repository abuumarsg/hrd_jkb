			...
			<div class="col-md-12">
				<div class="box box-success">
					<div class="box-header with-border">
						<h3 class="box-title"><i class="fa fa-list"></i> Setting Tunjangan <?=$nama?></h3>
											...
											<div class="collapse" id="add">
												...
												<div class="col-md-8">
													<form id="form_add">
														...
														<div class="form-group">
															<input type="hidden" name="induk_tunjangan" value="<?php echo $this->codegenerator->decryptChar($this->uri->segment(4));?>">
															<label>Nama Tunjangan</label>
															<input type="text" placeholder="Masukkan Nama" id="data_name_add" name="nama" class="form-control field" value="<?=$nama?>" required="required">
														</div>
														<div class="form-group">
															...
															<select class="form-control select2" name="karyawan[]" id="data_karyawan_add" style="width: 100%;" multiple="multiple">
																<?php
																	...
																	$emp = $this->model_karyawan->getPilihKaryawanMutasi($where);
																	echo '<option value="all">Pilih Semua</option>';
																	foreach ($emp as $j) {
																		echo '<option value="'.$j->id_karyawan.'">'.$j->nama.' - '.$j->nama_jabatan.'</option>';
																	}
																?>
															</select>
														</div>
      ...
      <div class="modal-content">
         ...
         <div class="modal-footer">
            <?php if (in_array($access['l_ac']['edt'], $access['access'])) {
               echo '<button type="submit" id="button_edit" class="btn btn-info" onclick="edit_modal()"><i class="fa fa-edit"></i> Edit</button>';
            }?>
               ...
               <div class="form-group">
               	...
               	<select class="form-control select2" name="karyawan[]" id="data_karyawan_edit" style="width: 100%;" multiple="multiple">
               		<?php
						$whereE = null;
						if($level != 0 ){
							$whereE = 'emp.golongan=1';	
						}
						echo '<option value=""></option>';
						$emp = $this->model_karyawan->getPilihKaryawanMutasi($whereE);
						echo '<option value="all">Pilih Semua</option>';
						foreach ($emp as $j) {
							echo '<option value="'.$j->id_karyawan.'">'.$j->nama.' - '.$j->nama_jabatan.'</option>';
						}
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		ajax: {
			url: "<?php echo base_url('master/master_tunjangan/view_all/'.$this->uri->segment(4))?>",
			type: 'POST',
		...
		$('#btn_tambah').click(function(){
			...
			refreshCode();
		select_data('data_induk_grade_add',url_select,'master_induk_grade','kode_induk_grade','nama');
	...
	function view_modal(id) {
		...
		$('#button_edit').show();
		if(level != '0'){
			if(callback['create_by'] != callback['admin']){ 
				$('#button_edit').hide();
			}
		}
	}
  	function edit_modal() {
  		...
  		select_data('data_induk_grade_edit',url_select,'master_induk_grade','kode_induk_grade','nama');
  		...
  		// $('#data_jabatan_edit').val(callback['tunjangan']).trigger('change');
