<div class="content-wrapper">
	<section class="content-header">
		<h1>
			...
			<small>View Detail Periode Lembur</small>
		</h1>
		<ol class="breadcrumb">
			<li><a href="<?php echo base_url('pages/dashboard');?>"><i class="fas fa-tachometer-alt"></i> Dashboard</a></li>
			<li><a href="<?php echo base_url('pages/periode_lembur');?>"><i class="fa far fa-credit-card"></i> Data Periode</a></li>
			<li class="active">View Detail Periode Lembur</li>
		</ol>
	</section>
			...
			<div class="col-md-12">
				<div class="box box-success">
					<div class="box-header with-border">
						<h3 class="box-title"><i class="fa far fa-credit-card"></i> Daftar Detail <?php echo $periode['nama']; ?> </h3>
										...
										<div class="col-md-8">
											<form id="form_add">
												...
												<div class="form-group">
													...
													<?php 
													if($status_adm == 0 || $status_adm == 1 || $status_adm == 2 || $status_adm == 3 || $status_adm == 4){ ?>
														<select class="form-control select2" name="loker" id="data_loker_add" style="width: 100%;" required="required" onchange="get_child('add',this.value)"></select>
														<?php
													}else{
														?>
...
<script type="text/javascript">
	...
	/*wajib diisi*/
	var table="data_periode_lembur_detail";
	...
	$(document).ready(function(){
		...
		$('#table_data').DataTable( {
			ajax: {
				url: "<?php echo base_url('master/view_periode_lembur/view_all/'.$this->uri->segment(3))?>",
				type: 'POST',
		...
		$('#btn_tambah').click(function(){
			...
			if(key == 0){
				...
				if('<?php echo $status_adm; ?>' == '0' || '<?php echo $status_adm; ?>' == '1' || '<?php echo $status_adm; ?>' == '2' || '<?php echo $status_adm; ?>' == '3' || '<?php echo $status_adm; ?>' == '4'){
					select_data('data_loker_add',url_select,'master_loker','kode_loker','nama');
				}else{
	...
	function refreshCode() {
		kode_generator("<?php echo base_url('master/view_periode_lembur/kode');?>",'data_kode_add');
	}
	function get_child(usage,value) {
		var data={kode_loker:value};
		var callback=getAjaxData("<?php echo base_url('master/view_periode_lembur/child')?>",data);
	...
	function view_modal(id) {
		...
		var callback=getAjaxData("<?php echo base_url('master/view_periode_lembur/view_one')?>",data);  
		$('#view').modal('show');
		$('.header_data').html(callback['nama']);
		$('#data_kode_view').html(callback['kode']);
		$('#data_loker_view').html(callback['loker']);
		$('#data_detail_view').html(callback['bagian']);
	...
	function delete_modal(id) {
		...
		$('#delete #form_delete .modal-body p').html('Apakah anda yakin akan menghapus data dengan kode <b id="data_name_delete" class="header_data">'+callback['kode']+'</b> ?');
	}
	...
	function do_edit(){
		if($("#form_edit")[0].checkValidity()) {
			submitAjax("<?php echo base_url('master/edt_periode_lembur_detail')?>",'edit','form_edit',null,null);
			$('#table_data').DataTable().ajax.reload();
		}else{
			notValidParamx();
		} 
	...
	function do_add(){
		if($("#form_add")[0].checkValidity()) {
			submitAjax("<?php echo base_url('master/add_periode_lembur_detail')?>",null,'form_add',null,null);
