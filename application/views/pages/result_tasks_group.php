  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
       <a class="back" onclick="window.history.back()"><i class="fa fa-chevron-circle-left "></i></a> <i class="fa fa-line-chart"></i> Daftar Hasil Penilaian
      </h1>
      <ol class="breadcrumb">
        ...
        <div class="col-md-12">
          <div class="box box-warning">
            <div class="box-header with-border">
              <h3 class="box-title"><i class="fa fa-line-chart"></i> Daftar Hasil Penilaian</h3>
              <div class="box-tools pull-right">
                <?php 
                  $agd=$this->db->get_where('agenda',array('kode_agenda'=>$this->uri->segment(3)))->row_array();
                  if ($agd != "") {
                    echo '<label class="label label-success">'.$agd['nama_agenda'].' Semester '.$agd['semester'].'</label>';
                  }
                ?>
                  ...
                  <table id="example1" class="table table-bordered table-striped  table-hover">
                    <thead>
                      <tr>
                        ...
                        <th class="text-center">Nilai Sikap dan Perilaku</th>
                        <th class="text-center">Nilai Pencapaian OS</th>
                      ...
                      <?php
                        ...
                        foreach ($res as $k) {
                          ...
                          $naa[$k]=$na[$k][0]+$na[$k][1]+$na[$k][2];
                            ...
                            echo '<tr>
                                  <td width="3%">'.$n1.'.</td>
                                  <td><a href="'.base_url('pages/report_group/'.$nmtb.'/'.$k).'">';
                                    $con=$this->db->get_where('master_konversi_nilai',array('status'=>'aktif'))->result();
                                    foreach ($con as $c) {
                                      if (number_format($naa[$k],2) >= $c->awal && number_format($naa[$k],2) <= $c->akhir) {
                                        if ($c->warna != NULL) {
                                  ...
                                  <td class="text-center">'.number_format($na[$k][1],2,',',',').'</td>
                                  <td class="text-center">'.number_format($na[$k][2],2,',',',').'</td>
                                  <td class="text-center">'.number_format($naa[$k],2,',',',').'</td>
                                  </tr>';
